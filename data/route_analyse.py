...
def init_data():
    ...
    with open('bus-stops.json') as f:
        process_stops(json.load(f))
    print('ok')

    print('processing buses... ', end='')
    with open('bus-services.json') as f:
        process_services(json.load(f))

    for root, _, files in walk('bus-services'):
...
def write_data():
    ...
    with open('processed.min.json', 'w') as f:
        json.dump(data, f, separators=(',', ':'), sort_keys=True)
    print('ok')


def load_data():
    with open('processed.min.json') as f:
        return json.load(f)
